...
pub trait Visitor {
    ...
    fn visit_external_crate(&mut self, external_crate: &ExternalCrate) {
        visit_external_crate(self, external_crate);
    }
    ...
    fn visit_deprecation(&mut self, deprecation: &Deprecation) {
        visit_deprecation(self, deprecation);
    }
    ...
    fn visit_discriminant(&mut self, discriminant: &Discriminant) {
        visit_discriminant(self, discriminant);
    }
...
pub fn visit_identifiable(vis: &mut (impl Visitor + ?Sized), identifiable: &Identifiable) {
    match identifiable {
        Identifiable::Item(_0) => {
            if let Some(_0) = _0.upgrade().as_ref() {
                vis.visit_item(_0);
            }
        }
        Identifiable::Summary(_0) => {
            vis.visit_item_summary(_0);
        }
        _ => (),
    }
}
pub fn visit_external_crate(vis: &mut (impl Visitor + ?Sized), external_crate: &ExternalCrate) {}
...
pub fn visit_id(vis: &mut (impl Visitor + ?Sized), id: &Id) {}
...
pub fn visit_span(vis: &mut (impl Visitor + ?Sized), span: &Span) {}
pub fn visit_deprecation(vis: &mut (impl Visitor + ?Sized), deprecation: &Deprecation) {}
...
pub fn visit_dyn_trait(vis: &mut (impl Visitor + ?Sized), dyn_trait: &DynTrait) {
    for trait_ in &dyn_trait.traits {
        vis.visit_poly_trait(trait_);
    }
}
pub fn visit_poly_trait(vis: &mut (impl Visitor + ?Sized), poly_trait: &PolyTrait) {
    vis.visit_path(&poly_trait.trait_);
    for generic_param in &poly_trait.generic_params {
        vis.visit_generic_param_def(generic_param);
    }
}
...
pub fn visit_type_binding_kind(
    ...
    match type_binding_kind {
        TypeBindingKind::Equality(_0) => {
            vis.visit_term(_0);
        }
        TypeBindingKind::Constraint(_0) => {
            for _0 in _0 {
                vis.visit_generic_bound(_0);
            }
        }
        _ => (),
    }
...
pub fn visit_item(vis: &mut (impl Visitor + ?Sized), item: &Item) {
    match item {
        ...
        Item::Function(_0) => {
            vis.visit_function(_0);
        }
        Item::Trait(_0) => {
            vis.visit_trait(_0);
        }
        Item::TraitAlias(_0) => {
            vis.visit_trait_alias(_0);
        }
        Item::Impl(_0) => {
            vis.visit_impl(_0);
        }
        Item::Typedef(_0) => {
            vis.visit_typedef(_0);
        }
        Item::OpaqueTy(_0) => {
            vis.visit_opaque_ty(_0);
        }
        Item::Constant(_0) => {
            vis.visit_constant_item(_0);
        }
        Item::Static(_0) => {
            vis.visit_static(_0);
        }
        Item::Macro { name, meta, expr } => {
            vis.visit_item_meta(meta);
        }
        Item::ProcMacro(_0) => {
            vis.visit_proc_macro(_0);
        }
        Item::Primitive(_0) => {
            vis.visit_primitive(_0);
        }
        Item::AssocConst {
            meta,
            type_,
...
pub fn visit_union(vis: &mut (impl Visitor + ?Sized), union: &Union) {
    ...
    for field in &union.fields {
        vis.visit_identifiable(field);
    }
    for imp in &union.impls {
...
pub fn visit_discriminant(vis: &mut (impl Visitor + ?Sized), discriminant: &Discriminant) {}
...
pub fn visit_generics(vis: &mut (impl Visitor + ?Sized), generics: &Generics) {
    ...
    for where_predicate in &generics.where_predicates {
        vis.visit_where_predicate(where_predicate);
    }
}
...
pub fn visit_type(vis: &mut (impl Visitor + ?Sized), ty: &Type) {
    match ty {
        Type::ResolvedPath(_0) => {
            vis.visit_path(_0);
        }
        Type::DynTrait(_0) => {
            vis.visit_dyn_trait(_0);
        }
        Type::FunctionPointer(_0) => {
            vis.visit_function_pointer(_0);
        }
        Type::Tuple(_0) => {
            for _0 in _0 {
                vis.visit_type(_0);
...
pub fn visit_function_pointer(
    ...
    for generic_param in &function_pointer.generic_params {
        vis.visit_generic_param_def(generic_param);
    }
...
pub fn visit_trait(vis: &mut (impl Visitor + ?Sized), trait_: &Trait) {
    ...
    for implementation in &trait_.implementations {
        vis.visit_identifiable(implementation);
    }
}
pub fn visit_trait_alias(vis: &mut (impl Visitor + ?Sized), trait_alias: &TraitAlias) {
    vis.visit_generics(&trait_alias.generics);
    for param in &trait_alias.params {
        vis.visit_generic_bound(param);
    }
}
